...
export class PodStore {
  ...
  // moderation-events.json: [{ ts, kind }, ...] — one entry per inbound item
  // intake.mjs actually refused because of a block, not because a block
  // merely exists. This is the only place that number can be known at all:
  // a blocked actor's post never reaches Ailo, so the client has nothing to
  // count. `kind` is 'blocked-actor' (isBlocked() matched a sender or
  // author) or 'blocked-domain' (isBlocked() matched an object's origin).
  // Capped at 5,000 entries and pruned past 30 days on every write so one
  // hostile flood can't grow this file unbounded — the weekly summary this
  // feeds only ever looks back 7-14 days.
  recordModerationEvent(kind) {
    const MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;
    const MAX_ENTRIES = 5_000;
    const cutoff = Date.now() - MAX_AGE_MS;
    const events = this.read('moderation-events.json', [])
      .filter(e => e && typeof e.ts === 'number' && e.ts >= cutoff);
    events.push({ ts: Date.now(), kind });
    this.write('moderation-events.json', events.slice(-MAX_ENTRIES));
  }

  // Counts by kind for events within the last `days` days — the raw material
  // for the Ailo weekly moderation summary (see mastoapi.mjs's
  // /api/v1/ailo/moderation/intake-stats).
