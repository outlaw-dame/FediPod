...
export class PodStore {
  ...
  // moderation-events.json: [{ ts, kind, domain }, ...] — one entry per
  // inbound item intake.mjs actually refused because of a block, not because
  // a block merely exists. This is the only place that number can be known
  // at all: a blocked actor's post never reaches Ailo, so the client has
  // nothing to count. `kind` is 'blocked-actor' (isBlocked() matched a
  // sender or author) or 'blocked-domain' (isBlocked() matched an object's
  // origin); `domain` is the refused item's origin hostname, for the "top
  // domains" breakdown below. Capped at 5,000 entries and pruned past 30
  // days on every write so one hostile flood can't grow this file
  // unbounded — the weekly summary this feeds only ever looks back 7-14 days.
  recordModerationEvent(kind, domain = null) {
    const MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;
    const MAX_ENTRIES = 5_000;
    const cutoff = Date.now() - MAX_AGE_MS;
    const events = this.read('moderation-events.json', [])
      .filter(e => e && typeof e.ts === 'number' && e.ts >= cutoff);
    events.push({ ts: Date.now(), kind, domain: domain || null });
    this.write('moderation-events.json', events.slice(-MAX_ENTRIES));
  }

  // Counts by kind for events within the last `days` days, plus the domains
  // behind the most refusals — the raw material for the Ailo weekly
  // moderation summary (see mastoapi.mjs's /api/v1/ailo/moderation/intake-stats).
  getModerationStats(days = 7) {
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    const events = this.read('moderation-events.json', []).filter(e => e && e.ts >= cutoff);
    ...
    const topDomains = [...domainCounts.entries()]
      ...
      .slice(0, 5);
    return {
      blockedActor: events.filter(e => e.kind === 'blocked-actor').length,
      blockedDomain: events.filter(e => e.kind === 'blocked-domain').length,
      topDomains,
    };
  }
  ...
  // followed tags: string[] of normalized names (no leading '#'). Nothing
  // federates on this — it only narrows what the client's own hashtag
  // timeline/UI shows.
  getFollowedTags() { return this.read('followed-tags.json', []); }
  setFollowedTags(t) { this.write('followed-tags.json', t); }
  // featured tags: [{ id, name }] surfaced on the profile. statuses_count/
  // last_status_at are derived from stored statuses at read time rather than
  // kept here, so they can't drift out of sync with the posts themselves.
  getFeaturedTags() { return this.read('featured-tags.json', []); }
  setFeaturedTags(t) { this.write('featured-tags.json', t); }

  // provider credentials: { openai?, gemini?, safe_browsing?, klipy?, deepl?, libretranslate?: "<api key>" }.
  // Saved through Ailo's provider-key UI — checked before the matching
  // AP_*_API_KEY env var (see credentialFor() in mastoapi.mjs). Same file
  // permissions as every other document here (private/ap-state, 0600):
  // there is no separate encryption layer for this one, same as the rest.
  getProviderCredentials() { return this.read('provider-credentials.json', {}); }
  setProviderCredentials(c) { this.write('provider-credentials.json', c); }
  getTranslationSettings() { return this.read('translation-settings.json', {}); }
  setTranslationSettings(s) { this.write('translation-settings.json', s); }
  ...
  getCustomFeeds() { return this.read('custom-feeds.json', []); }
  setCustomFeeds(f) { this.write('custom-feeds.json', f); }
