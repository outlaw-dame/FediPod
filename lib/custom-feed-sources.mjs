...
// /api/v1/* specifically — the literal substring "FediPod", deliberately,
// since e.g. "fedipod/0.5" passes and "FediPod" alone does not. Their
// public SSE stream (what publicfeed.mjs already consumes) has no such
// block, so that path is untouched — but sending a different UA just for
// these calls to route around a host's own targeted block on this client
// would be evading a decision they made about us on purpose, not fixing a
// bug. Polling several ordinary well-connected instances instead is less
// complete than one global relay would have been, but it is honest, and
// several general-audience instances together — tried in turn, falling
// through on refusal — emulate a relay's breadth reasonably well.
//
// A feed built on semantic keywords/topic phrases rather than a hashtag has
// the same cold-start gap for a harder reason: there is no Follow-shaped
// primitive for a keyword the way relay-tags.mjs follows a hashtag actor
// (BuzzRelay only relays real hashtags), and no fediverse-wide keyword
// search exists unauthenticated — confirmed by hand that Mastodon's own
// /api/v2/search answers empty statuses without a login. _semanticBackfill
// approximates it the same way a relay would: pull each candidate
// instance's own recent public posts and decide relevance here — by real
// embedding similarity when an AI provider is configured (the same
// matchByEmbedding path mastoapi.mjs's /api/v1/ailo/ai/filters/match and
// /ai/search routes already use), or a literal-phrase fallback otherwise,
// same as exactCustomFeedMatch's own fallback client-side.
...
import { aiBackendFor as realAiBackendFor } from './mastoapi.mjs';
...
const PUBLIC_POOL_PER_INSTANCE = 20;    // recent public posts requested per instance per sweep
const MAX_SEMANTIC_CANDIDATES = 80;     // total posts pulled before matching — bounds the embedding call's size
const MAX_SEMANTIC_BACKFILL_PER_SWEEP = 8;  // dereference budget — its own pool; embedding calls cost real money

// Union of every custom feed's semantic keywords/topic phrases — the same
// shape as relay-tags.mjs's customFeedHashtags, one level over.
function customFeedSemanticKeywords(store) {
  const keywords = new Set();
  for (const feed of store.getCustomFeeds()) for (const kw of feed.semanticKeywords || []) keywords.add(kw);
  return keywords;
}
...
export class CustomFeedSources {
  constructor({
    ...
    aiBackendFor = realAiBackendFor,
  }) {
    Object.assign(this, { store, intake, agent, log, resolveHandle, fetcher, aiBackendFor });
  ...
  // Reverse-chronological top-up for semantic-keyword custom feeds — see
  // the header comment for why there's no Follow or search-endpoint
  // shortcut here the way there is for hashtags. Pulls a bounded pool of
  // recent public posts from each candidate instance's own local timeline
  // (unauthenticated everywhere, unlike the federated public timeline,
  // which several large instances refuse without a login — confirmed by
  // hand), then decides relevance once against the whole pool rather than
  // per instance.
  async _semanticBackfill(known, budget) {
    const keywords = customFeedSemanticKeywords(this.store);
    let added = 0;
    if (!keywords.size || budget <= 0) return { added, budget };

    const candidates = [];
    for (const instance of BACKFILL_INSTANCES) {
      if (candidates.length >= MAX_SEMANTIC_CANDIDATES) break;
      const host = new URL(instance).host;
      if (this._hostQuiet(host)) continue;
      let list;
      try {
        const url = `${instance}/api/v1/timelines/public?local=true&limit=${PUBLIC_POOL_PER_INSTANCE}`;
        const res = this.fetcher === globalThis.fetch
          ? await safeFetch(url, { headers: { accept: 'application/json' } })
          : await this.fetcher(url, { headers: { accept: 'application/json' } });
        if (res.status >= 400) { this._backOff(host); continue; }
        ...
        this.log(`custom-feed-sources semantic-backfill via ${host}: ${e.message}`);
        this._backOff(host);
        continue;
      }
      const { sanitizeHtml } = await import('./wire.mjs');
      for (const st of Array.isArray(list) ? list : []) {
        const noteId = st?.uri;
        if (!noteId || known.has(noteId) || this.store.isBlocked(noteId)) continue;
        const text = sanitizeHtml(st?.content || '').replace(/<[^>]+>/g, ' ').trim();
        if (!text) continue;
        candidates.push({ noteId, text });
        // Marked known as soon as it's queued, not just once actually stored:
        // several candidate instances can (and often do) hand back the same
        // widely-federated post, and this is what keeps it a single candidate
        // instead of one per instance that happened to have it.
        known.add(noteId);
      }
    }
    if (!candidates.length) return { added, budget };

    // Real embedding similarity when a provider is configured — the exact
    // provider-precedence rule mastoapi.mjs's own AI routes use, imported
    // rather than re-decided here so the two never drift apart. Falls back
    // to a literal-phrase match (same fallback exactCustomFeedMatch already
    // applies client-side) with no provider configured, or if the call
    // itself fails — a feed still gets *something* either way.
    let matchedIds = null;
    const ai = this.aiBackendFor(this.store);
    if (!ai.error) {
      try {
        const queries = [...keywords].map((kw) => ({ id: kw, text: kw }));
        const documents = candidates.map((c) => ({ id: c.noteId, text: c.text }));
        const matches = await ai.backend.matchByEmbedding({ apiKey: ai.apiKey, queries, documents });
        matchedIds = new Set(matches.map((m) => m.documentId));
      } catch (e) {
        this.log(`custom-feed-sources semantic-backfill embedding: ${e.message}`);
      }
    }
    if (!matchedIds) {
      const needles = [...keywords].map((kw) => kw.toLocaleLowerCase());
      matchedIds = new Set(
        candidates
          .filter((c) => needles.some((needle) => c.text.toLocaleLowerCase().includes(needle)))
          .map((c) => c.noteId),
      );
    }
    ...
    for (const candidate of candidates) {
      if (budget <= 0) break;
      if (!matchedIds.has(candidate.noteId)) continue;
      // Verified by dereference at the note's own origin, same as tagBackfill —
      // the pulled timeline JSON and the embedding match are both only ever
      // pointers to go check the real thing, never trusted as content on their own.
      const note = await this.intake.fetchAP(candidate.noteId).catch(() => null);
      if (!note || note.id !== candidate.noteId || !isContentType(note.type)) continue;
      const author = authorOf(note);
      if (!author || this.store.isBlocked(author)) continue;
      if (!this.store.getActors()[author]) await this.intake.fetchAP(author).catch(() => {});
      const { attachmentsOf, sanitizeHtml } = await import('./wire.mjs');
      const attachments = attachmentsOf(note);
      this.store.addStatus({
        noteId: candidate.noteId, actor: author, content: sanitizeHtml(note.content),
        published: note.published, inReplyTo: note.inReplyTo, kind: 'tag',
        ...(attachments.length ? { attachments } : {}),
      });
      known.add(candidate.noteId);
      added++;
      budget--;
    }
    return { added, budget };
  ...
  async sweep() {
    ...
    const semanticBackfill = await this._semanticBackfill(known, MAX_SEMANTIC_BACKFILL_PER_SWEEP)
      .catch((e) => { this.log(`custom-feed-sources semantic-backfill: ${e.message}`); return { added: 0 }; });
    this.lastSemanticBackfilled = semanticBackfill.added;

    const accounts = this.accountsToSweep();
    let budget = MAX_NEW_PER_SWEEP;
    let added = backfill.added + semanticBackfill.added;
    ...
    if (added) {
      const notes = [
        backfill.added ? `${backfill.added} tag-backfilled` : null,
        semanticBackfill.added ? `${semanticBackfill.added} semantic-backfilled` : null,
      ].filter(Boolean).join(', ');
      this.log(`custom-feed-sources: +${added}${notes ? ` (${notes})` : ''} from ${accounts.length} account(s)`);
    }
  }
}
