...
import {
  isAiEnabled, translateText, suggestHashtags, suggestModeration, matchByEmbedding, recommendTopics,
} from './ai.mjs';
...
function hashtagsIn(text) {
  return (String(text || '').match(/#[a-z0-9_]+/gi) || []).map(t => t.slice(1).toLowerCase());
}
...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/api/v1/ailo/ai/search' && req.method === 'POST') {
      ...
      // client that doesn't ask for it sees no behavior change.
      const body = await readBody(req);
      ...
      try {
        ...
        let semanticIds = [];
        if (documents.length) {
          const matches = await matchByEmbedding({ queries: [{ id: 'q', text: q }], documents });
    ...
    if (pathname === '/api/v1/ailo/ai/recommendations' && req.method === 'GET') {
      ...
      const pinnedText = all
        ...
        .filter(Boolean);
      try {
        const topics = await recommendTopics({ ownTags, pinnedText });
