...
import { isAiEnabled, translateText, suggestHashtags, suggestModeration, matchByEmbedding } from './ai.mjs';
...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    // --- Ailo/FediPod extension: OpenAI-backed features. Not standard
    // Mastodon API — disabled (503) unless AP_OPENAI_API_KEY is set. See
    // lib/ai.mjs and .env.example. `status` answers regardless of whether AI
    // is enabled (that's the point of it); everything else under this prefix
    // is gated by the check right after it.
    if (pathname === '/api/v1/ailo/ai/status' && req.method === 'GET') {
      return send(200, { enabled: isAiEnabled() });
    }
    if (pathname.startsWith('/api/v1/ailo/ai/') && !isAiEnabled()) {
      return send(503, { error: 'AI features are not configured on this agent' });
    }
    if (pathname === '/api/v1/ailo/ai/translate' && req.method === 'POST') {
      const body = await readBody(req);
      try {
        const translated = await translateText({ text: body.text, targetLang: body.target_lang });
    ...
    if (pathname === '/api/v1/ailo/ai/hashtags/suggest' && req.method === 'POST') {
      const body = await readBody(req);
      // Followed/featured tags are not real server-side data yet — both
      // /api/v1/followed_tags and accounts/:id/featured_tags are still dead
      // stubs above — so personalization is limited to the account's own past
      // hashtag use, scanned out of its own published posts' text. There is
      // no stored hashtag-extraction pipeline to read from instead: intake.mjs
      // passes AS2 Hashtag tag entries through untouched today.
      const ownTags = [...new Set(
        this.store.getStatuses()
          .filter(s => s.kind === 'post')
          .flatMap(s => (String(s.content || s.text || '').match(/#[a-z0-9_]+/gi) || []))
          .map(t => t.slice(1).toLowerCase()),
      )].slice(0, 50);
      try {
        const hashtags = await suggestHashtags({ text: body.text, ownTags });
    ...
    if (pathname === '/api/v1/ailo/ai/moderation/suggest' && req.method === 'POST') {
      ...
      try {
        const suggestions = await suggestModeration({ filterTitles, blockedDomains: domains, mutedCount });
    ...
    if (pathname === '/api/v1/ailo/ai/filters/match' && req.method === 'POST') {
      // Generic embedding-similarity matcher: used now for the opt-in OpenAI
      // backend on semantic filter keywords (client sends the filter keyword
      // as the query, candidate status text as documents), reusable later for
      // hybrid search without a new endpoint.
      const body = await readBody(req);
      try {
        const matches = await matchByEmbedding({ queries: body.queries, documents: body.documents });
