...
import { COLLECTION_SOURCES, loadCollectionSource } from './collection-sources.mjs';
import { normalizeAttributionDomains } from './creator-attribution.mjs';
...
const MAX_PINNED_STATUSES = 5;
const MAX_COLLECTIONS = 100;
const TAG_NAME = /^(?=.*[\p{L}\p{M}_])[\p{L}\p{M}\p{N}_]+$/u;
const IMPORT_ACTOR_TYPES = new Set(['Person', 'Group', 'Service', 'Application', 'Organization']);
const MASTODON_VERSION = '4.6.0 (compatible; fedipod)';

export function normalizeTagName(value) {
  let name;
  try { name = decodeURIComponent(String(value || '')).trim().replace(/^#/, '').normalize('NFC').toLowerCase(); }
  catch { return null; }
  return name && [...name].length <= 100 && TAG_NAME.test(name) ? name : null;
}

export function hashtagsIn(value) {
  const text = htmlToText(value || '');
  const found = [];
  const seen = new Set();
  for (const match of text.matchAll(/(^|[^\p{L}\p{M}\p{N}_])#([\p{L}\p{M}\p{N}_]+)/gu)) {
...
function collectionJson(c) {
  return {
    id: c.id, account_id: c.accountId, uri: c.uri, url: c.url,
    name: c.name, description: c.description || '', language: c.language || null,
    local: c.local !== false, sensitive: !!c.sensitive, discoverable: !!c.discoverable,
    tag: c.tagName ? { name: c.tagName, url: `${c.url}#${encodeURIComponent(c.tagName)}` } : null,
    source_url: c.sourceUrl || null, source_page: c.sourcePage || null,
    source_kind: c.sourceKind || null,
    item_count: (c.items || []).filter(i => i.state === 'accepted' || i.state === 'pending').length,
    items: (c.items || []).map(i => ({
      id: i.id, account_id: i.accountId, state: i.state, created_at: i.createdAt,
    })),
    created_at: c.createdAt, updated_at: c.updatedAt,
  };
}

function collectionInput(body, current = {}) {
  const name = String(body.name ?? current.name ?? '').trim();
  const description = String(body.description ?? current.description ?? '').trim();
  const language = String(body.language ?? current.language ?? '').trim() || null;
  const tagName = body.tag_name == null ? (current.tagName || null) : normalizeTagName(body.tag_name);
  if (!name || [...name].length > 40) return { error: 'name must be between 1 and 40 characters' };
  if ([...description].length > 100) return { error: 'description must be 100 characters or fewer' };
  if (body.tag_name && !tagName) return { error: 'tag_name is invalid' };
  if (language && !/^[a-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(language)) return { error: 'language is invalid' };
  return {
    name, description, language, tagName,
    sensitive: body.sensitive == null ? !!current.sensitive : body.sensitive === true || body.sensitive === 'true',
    discoverable: body.discoverable == null ? !!current.discoverable : body.discoverable === true || body.discoverable === 'true',
  };
}
...
export class MastoApi {
  constructor({ agent, log = console.log, allowed = null, collectionSourceLoader = loadCollectionSource,
    creatorPreview = null, ai = null, safeBrowsing = null, providerSecrets = null }) {
    this.agent = agent;
    this.log = log;
    this.allowed = allowed;             // authorities a redirect_uri may name
    this.collectionSourceLoader = collectionSourceLoader;
    this.creatorPreview = creatorPreview;
    this.ai = ai;
    this.safeBrowsing = safeBrowsing;
    this.providerSecrets = providerSecrets;
    this.credentialTests = [];
  ...
  followedTagNames() {
    return [...new Set((this.agent.tagfeed?.config()?.tags || [])
      .map(normalizeTagName).filter(Boolean))].slice(0, MAX_FOLLOWED_TAGS);
  }

  featuredTags() {
    const seen = new Set();
    return this.store.read('featured-tags.json', []).filter(t => {
      const name = normalizeTagName(t?.name);
      if (!name || seen.has(name) || typeof t?.id !== 'string') return false;
      t.name = name;
      seen.add(name);
      return true;
  ...
  statusTagNames(s) {
    return hashtagsIn([s.text, s.source?.content, s.title, s.spoiler, s.content]
      .filter(Boolean).join(' '));
  }

  tagUrl(name) { return `${this.urls.profileHtml}#${encodeURIComponent(name)}`; }
  ...
  tagHistory(name) {
    ...
    const rows = Array.from({ length: 7 }, (_, offset) => {
      ...
      const matches = this.store.getStatuses().filter(s => {
        const at = Date.parse(s.published || '');
        return at >= start.getTime() && at < end.getTime() && this.statusTagNames(s).includes(name);
      });
      return {
        day: String(Math.floor(start.getTime() / 1000)),
        uses: String(matches.length),
        accounts: String(new Set(matches.map(s => s.actor).filter(Boolean)).size),
      };
    });
    return rows;
  }
  ...
  tagJson(name) {
    return {
      id: name, name, url: this.tagUrl(name), history: this.tagHistory(name),
      following: this.followedTagNames().includes(name),
      featured: this.featuredTags().some(t => t.name === name),
    };
  }

  featuredTagJson(tag) {
    const own = this.store.getStatuses().filter(s => s.actor === this.urls.actor
      && this.statusTagNames(s).includes(tag.name));
    const latest = own.map(s => s.published).filter(Boolean).sort().at(-1) || null;
    return {
      id: tag.id, name: tag.name, url: this.tagUrl(tag.name),
      statuses_count: String(own.length),
      last_status_at: latest ? latest.slice(0, 10) : null,
    };
  }

  async replaceFeaturedTags(next, previous) {
    this.store.write('featured-tags.json', next);
    try { await this.agent.publisher.publishFeaturedTags(); }
    catch (error) {
      this.store.write('featured-tags.json', previous);
      await this.agent.publisher.publishFeaturedTags().catch(() => {});
      throw error;
    }
  }
  ...
  credentialAccount() {
    ...
    return {
      ...
      source: {
        ...
        note: cfg.summary || '',
        fields: (cfg.fields || []).map(f => ({ name: f.name, value: f.value })),
        attribution_domains: cfg.attributionDomains || [],
      },
    };
  }
  ...
  cardJson(card) {
    if (!card) return null;
    const authors = (card.authors || []).map(author => ({
      name: author.name || (author.actor ? this.account(author.actor).display_name : ''),
      url: author.url || (author.actor ? this.account(author.actor).url : ''),
      account: author.actor ? this.account(author.actor) : null,
    }));
    return {
      url: card.url, title: card.title || '', description: card.description || '',
      type: card.type || 'link', authors,
      author_name: authors[0]?.name || '', author_url: authors[0]?.url || '',
      provider_name: card.providerName || '', provider_url: card.providerUrl || '',
      html: '', width: 0, height: 0, image: null, embed_url: '', blurhash: null,
      missing_attribution: !!card.missingAttribution,
    };
  }
  ...
  status(s, { all, filterContext = 'thread', filters = null, quoteDepth = 0 } = {}) {
    // Link-card discovery is bounded and asynchronous. The first render queues
    // it; a later timeline refresh receives the persisted result.
    this.creatorPreview?.enqueue(s);
    const statuses = all || this.store.getStatuses();
    const replies = statuses.filter(x => x.inReplyTo === s.noteId).length;
    const quoted = s.quoteOf && quoteDepth < 1
      ? statuses.find(x => x.noteId === s.quoteOf) : null;
    const automatic = s.quotePolicy === 'nobody' ? []
      : s.quotePolicy === 'followers' ? ['followers'] : ['public'];
    return {
      ...
      tags: this.statusTagNames(s).map(name => ({ name, url: this.tagUrl(name) })),
      emojis: (s.emojis || []).map(e => ({
        shortcode: e.shortcode, url: e.url, static_url: e.url, visible_in_picker: false,
      })),
      card: this.cardJson(s.card), poll: s.poll ? this.pollJson(s) : null,
      quote: s.quoteOf ? {
        state: s.quoteState || 'pending',
        quoted_status: (s.quoteState || 'pending') === 'accepted' && quoted
          ? this.status(quoted, { all: statuses, filterContext, filters, quoteDepth: quoteDepth + 1 }) : null,
      } : null,
      quote_approval: s.actor === this.urls.actor ? {
        automatic, manual: [], current_user: s.quotePolicy === 'nobody' ? 'denied' : 'automatic',
      } : null,
      quotes_count: statuses.filter(x => x.quoteOf === s.noteId && x.quoteState === 'accepted').length,
    };
  }
  ...
  pushNotify(n) {
    ...
    const verbs = {
      ...
      quote: 'quoted your post', quoted_update: 'updated a quote of your post',
  ...
  notification(n, { filters = null } = {}) {
    ...
    if (n.collectionId) {
      const c = [...this.store.getCollections(), ...this.store.getRemoteCollections()]
        .find(x => x.id === n.collectionId || x.uri === n.collectionId);
      if (c) out.collection = collectionJson(c);
    }
    return out;
  }
  ...
  async resolveCollectionImport(accounts) {
    const resolved = new Array(accounts.length);
    let cursor = 0;
    const worker = async () => {
      for (;;) {
        const index = cursor++;
        ...
        let doc = null;
        if (candidate.url) {
          doc = await this.agent.intake.fetchAP(candidate.url).catch(() => null);
          try {
            if (!doc?.id || new URL(doc.id).origin !== new URL(candidate.url).origin) doc = null;
          } catch { doc = null; }
        }
        if ((!doc || !IMPORT_ACTOR_TYPES.has(doc.type)) && candidate.handle) {
          doc = await social.resolveHandle(this.agent, candidate.handle.replace(/^@/, '')).catch(() => null);
        }
        if (!doc?.id || !doc.inbox || !IMPORT_ACTOR_TYPES.has(doc.type)
          || doc.id === this.urls.actor || this.store.isBlocked(doc.id)) continue;
        resolved[index] = { actor: doc.id, inbox: doc.inbox };
      }
    };
    await Promise.all(Array.from({ length: Math.min(6, accounts.length) }, worker));
    const seen = new Set();
    return resolved.filter(Boolean).filter(item => {
  ...
  async importCollection(sourceUrl) {
    const pack = await this.collectionSourceLoader(sourceUrl);
    const resolved = await this.resolveCollectionImport(pack.accounts);
    if (!resolved.length) throw new Error('none of the source accounts could be verified over ActivityPub');
    const collections = this.store.getCollections();
    const existing = collections.find(c => c.sourceUrl === pack.url);
    if (!existing && collections.length >= MAX_COLLECTIONS) {
      throw new Error(`at most ${MAX_COLLECTIONS} collections are allowed`);
    }
    const now = new Date().toISOString();
    const input = collectionInput({
      name: pack.name, description: pack.description, discoverable: true,
    });
    if (input.error) throw new Error(input.error);
    const previous = existing ? structuredClone(existing) : null;
    const oldItems = new Map((existing?.items || []).map(item => [item.actor, item]));
    const present = new Set(resolved.map(item => item.actor));
    // A transient resolution failure must never turn into a mass opt-out. Only
    // remove absent accounts when every source row was verified successfully.
    const failedCount = pack.accounts.length - resolved.length;
    const removed = failedCount === 0
      ? (existing?.items || []).filter(item => !present.has(item.actor)) : [];
    const retainedAfterFailure = failedCount > 0
      ? (existing?.items || []).filter(item => !present.has(item.actor)) : [];
    const added = [];
    const items = resolved.map(({ actor, inbox }) => {
      const current = oldItems.get(actor);
      if (current) return { ...current, accountId: this.store.idFor(actor), inbox };
      const item = {
        id: crypto.randomBytes(8).toString('hex'), accountId: this.store.idFor(actor),
        actor, inbox, state: 'pending', createdAt: now,
      };
      added.push(item);
      return item;
    }).concat(retainedAfterFailure);
    const id = existing?.id || crypto.randomBytes(8).toString('hex');
    const collection = existing || {
      id, accountId: this.store.idFor(this.urls.actor), actor: this.urls.actor,
      uri: `${this.urls.collections}${id}`, url: `${this.urls.profileHtml}#collection-${id}`,
      createdAt: now,
    };
    Object.assign(collection, input, {
      sourceUrl: pack.url, sourcePage: pack.page, sourceKind: pack.kind,
      items, updatedAt: now,
    });
    const toInvite = items.filter(item => item.state === 'pending' && !item.requestId);
    if (this.store.getQueue().length + toInvite.length > 5000) {
      throw new Error('delivery queue is too full to schedule this Collection safely');
    }
    if (!existing) collections.unshift(collection);
    this.store.setCollections(collections);
    try { await this.agent.publisher.publishCollection(collection); }
    catch (error) {
      if (previous) collections[collections.indexOf(collection)] = previous;
      else collections.splice(collections.indexOf(collection), 1);
      this.store.setCollections(collections);
      throw new Error(`imported Collection could not be published: ${error.message}`);
    }
    let invitations = 0;
    const removedRequestIds = new Set(removed.map(item => item.requestId).filter(Boolean));
    const deliveryQueue = this.store.getQueue()
      .filter(entry => !removedRequestIds.has(entry.activity?.id));
    for (const item of toInvite) {
      const queued = await this.agent.publisher.requestCollectionFeature(collection, item, {
        queueOnly: true, queue: deliveryQueue,
      })
        .catch(error => { this.log(`imported collection feature request: ${error.message}`); return false; });
      delete item.inbox;
      if (queued) invitations++;
    }
    this.store.setQueue(deliveryQueue);
    this.store.setCollections(collections);
    // These request documents no longer describe an affiliation the source
    // endorses. Collection state is already safely published, so cleanup is
    // best-effort and cannot roll the refresh back.
    for (const requestId of removedRequestIds) {
      await this.agent.publisher.remote.delete(requestId)
        .catch(error => this.log(`retired collection feature request: ${error.message}`));
    }
    return {
      collections: [collectionJson(collection)], source_url: pack.url,
      account_count: collection.items.length, failed_count: failedCount,
      invitation_count: invitations, added_count: added.length, removed_count: removed.length,
      already_imported: Boolean(existing) && added.length === 0 && removed.length === 0,
    };
  }
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/api/v1/instance') {
      return send(200, {
        ...
        description: this.instanceBlurb(), email: '', version: MASTODON_VERSION,
    ...
    if (pathname === '/api/v2/instance') {
      return send(200, {
        domain: this.host, title: this.instanceTitle(), version: MASTODON_VERSION,
    ...
    // Ailo extensions. Provider API keys never cross this authenticated
    // boundary; the client supplies only a provider name and bounded inputs.
    if (pathname === '/api/v1/ailo/ai/status' && req.method === 'GET') {
      return send(200, this.ai?.status(this.safeBrowsing?.enabled()) || {
        enabled: false, providers: [], default_provider: null, models: {},
        safe_browsing: { enabled: !!this.safeBrowsing?.enabled() },
      });
    }
    if (pathname === '/api/v1/ailo/provider-credentials' && req.method === 'GET') {
      if (!this.providerSecrets) return send(503, { error: 'Provider credential storage is unavailable' });
      try { return send(200, this.providerSecrets.status()); }
      catch (error) { return send(Number(error.status) || 503, { error: error.message }); }
    }
    const credentialPath = /^\/api\/v1\/ailo\/provider-credentials\/(openai|gemini|safe_browsing)(?:\/(test))?$/.exec(pathname);
    if (credentialPath) {
      if (!this.providerSecrets) return send(503, { error: 'Provider credential storage is unavailable' });
      const provider = credentialPath[1];
      try {
        if (credentialPath[2] === 'test' && req.method === 'POST') {
          const now = Date.now();
          this.credentialTests = this.credentialTests.filter(time => now - time < 60_000);
          if (this.credentialTests.length >= 10) return send(429, { error: 'Too many credential tests; try again shortly' });
          this.credentialTests.push(now);
          const body = await readBody(req);
          const candidate = body.api_key == null || body.api_key === ''
            ? '' : this.providerSecrets.validate(body.api_key);
          const result = provider === 'safe_browsing'
            ? await this.safeBrowsing.testCredential(candidate)
            : await this.ai.testCredential(provider, candidate);
          return send(200, result);
        }
        if (req.method === 'PUT') {
          const body = await readBody(req);
          return send(200, this.providerSecrets.set(provider, body.api_key));
        }
        if (req.method === 'DELETE') return send(200, this.providerSecrets.delete(provider));
        return send(405, { error: 'method not allowed' });
      } catch (error) {
        this.log(`provider credential ${credentialPath[2] || req.method.toLowerCase()} failed for ${provider}: ${error.message}`);
        return send(Number(error.status) || 502, { error: error.message });
      }
    }
    if (pathname.startsWith('/api/v1/ailo/ai/') && req.method === 'POST') {
      if (!this.ai) return send(503, { error: 'AI service is unavailable' });
      const body = await readBody(req);
      try {
        if (pathname === '/api/v1/ailo/ai/translate') {
          return send(200, await this.ai.translate(body.provider, body.text, body.target_lang));
        }
        if (pathname === '/api/v1/ailo/ai/hashtags/suggest') {
          const result = await this.ai.hashtags(body.provider, body.text);
          const hashtags = [].concat(result.hashtags || []).map(value => normalizeTagName(value))
            .filter(Boolean).slice(0, 10);
          return send(200, { hashtags: [...new Set(hashtags)] });
        }
        if (pathname === '/api/v1/ailo/ai/moderation/suggest') {
          const blocklist = this.store.getBlocklist();
          const muted = this.store.getMuted();
          const result = await this.ai.moderation(body.provider, {
            blocked_domains: blocklist.domains,
            blocked_accounts: blocklist.actors.map(actor => this.account(actor).acct),
            muted_accounts: muted.actors.map(actor => this.account(actor).acct),
            filters: this.store.getFilters().map(filter => ({
              title: filter.title, keywords: (filter.keywords || []).map(keyword => keyword.keyword),
            })),
          });
          return send(200, result);
        }
        if (pathname === '/api/v1/ailo/ai/filters/match') {
          return send(200, await this.ai.matchFilters(body.provider, body.queries, body.documents));
        }
      } catch (error) {
        this.log(`ailo AI request failed: ${error.message}`);
        return send(Number(error.status) || 502, { error: error.message });
      }
    }
    if (pathname === '/api/v1/ailo/safety/urls/check' && req.method === 'POST') {
      if (!this.safeBrowsing) return send(503, { error: 'Google Safe Browsing is unavailable' });
      const body = await readBody(req);
      try { return send(200, await this.safeBrowsing.check(body.urls)); }
      catch (error) {
        this.log(`Safe Browsing request failed: ${error.message}`);
        return send(Number(error.status) || 502, { error: error.message });
      }
    }

    if (pathname === '/api/v1/accounts/verify_credentials') {
      return send(200, this.credentialAccount());
    }
    if (pathname === '/api/v1/profile' && req.method === 'GET') {
      const account = this.credentialAccount();
      return send(200, {
        ...account,
        attribution_domains: account.source.attribution_domains,
        formatted_note: account.note,
        formatted_fields: account.fields,
      });
    }

    // --- followed and featured hashtags ---
    if (pathname === '/api/v1/followed_tags' && req.method === 'GET') {
      const tags = this.followedTagNames().map(name => this.tagJson(name));
      const { items, headers } = this.page(tags, url,
        { limit: 100, max: 200, idOf: tag => tag.id });
      return send(200, items, headers);
    }

    if (pathname === '/api/v1/featured_tags') {
      if (req.method === 'GET') return send(200, this.featuredTags().map(t => this.featuredTagJson(t)));
      if (req.method === 'POST') {
        const body = await readBody(req);
        const name = normalizeTagName(body.name);
        if (!name) return send(422, { error: 'name must be a valid hashtag and cannot contain only numbers' });
        const previous = this.featuredTags();
        const existing = previous.find(t => t.name === name);
        if (existing) return send(200, this.featuredTagJson(existing));
        if (previous.length >= MAX_FEATURED_TAGS) {
          return send(422, { error: `at most ${MAX_FEATURED_TAGS} featured hashtags are allowed` });
        }
        const tag = { id: crypto.randomBytes(8).toString('hex'), name };
        try { await this.replaceFeaturedTags([...previous, tag], previous); }
        catch (error) { this.log(`featured tags: ${error.message}`); return send(502, { error: 'featured hashtags could not be published' }); }
        return send(200, this.featuredTagJson(tag));
      }
    }

    if (pathname === '/api/v1/featured_tags/suggestions' && req.method === 'GET') {
      const featured = new Set(this.featuredTags().map(t => t.name));
      const recent = this.store.getStatuses().filter(s => s.actor === this.urls.actor)
        .slice().sort((a, b) => String(b.published || '').localeCompare(String(a.published || '')));
      const names = [];
      const seen = new Set();
      for (const status of recent) {
        for (const name of this.statusTagNames(status)) {
          if (!featured.has(name) && !seen.has(name)) { seen.add(name); names.push(name); }
          if (names.length === 10) break;
        }
        if (names.length === 10) break;
      }
      return send(200, names.map(name => this.tagJson(name)));
    }

    const mFeaturedDelete = /^\/api\/v1\/featured_tags\/([a-f0-9]+)$/.exec(pathname);
    if (mFeaturedDelete && req.method === 'DELETE') {
      const previous = this.featuredTags();
      if (!previous.some(t => t.id === mFeaturedDelete[1])) return send(404, { error: 'Record not found' });
      try { await this.replaceFeaturedTags(previous.filter(t => t.id !== mFeaturedDelete[1]), previous); }
      catch (error) { this.log(`featured tags: ${error.message}`); return send(502, { error: 'featured hashtags could not be published' }); }
      return send(200, {});
    }

    const mTag = /^\/api\/v1\/tags\/([^/]+)(?:\/(follow|unfollow|feature|unfeature))?$/.exec(pathname);
    if (mTag) {
      const name = normalizeTagName(mTag[1]);
      if (!name) return send(404, { error: 'Record not found' });
      if (!mTag[2] && req.method === 'GET') return send(200, this.tagJson(name));
      if (req.method !== 'POST') return send(405, { error: 'POST expected' });
      if (mTag[2] === 'follow' || mTag[2] === 'unfollow') {
        const names = this.followedTagNames();
        if (mTag[2] === 'follow' && !names.includes(name)) {
          if (names.length >= MAX_FOLLOWED_TAGS) return send(422, { error: `at most ${MAX_FOLLOWED_TAGS} hashtags may be followed` });
          this.agent.tagfeed.setConfig({ tags: [...names, name] });
        } else if (mTag[2] === 'unfollow' && names.includes(name)) {
          this.agent.tagfeed.setConfig({ tags: names.filter(t => t !== name) });
        }
        return send(200, this.tagJson(name));
      }
      const previous = this.featuredTags();
      const existing = previous.find(t => t.name === name);
      let next = previous;
      if (mTag[2] === 'feature' && !existing) {
        if (previous.length >= MAX_FEATURED_TAGS) return send(422, { error: `at most ${MAX_FEATURED_TAGS} featured hashtags are allowed` });
        next = [...previous, { id: crypto.randomBytes(8).toString('hex'), name }];
      } else if (mTag[2] === 'unfeature' && existing) next = previous.filter(t => t.name !== name);
      if (next !== previous) {
        try { await this.replaceFeaturedTags(next, previous); }
        catch (error) { this.log(`featured tags: ${error.message}`); return send(502, { error: 'featured hashtags could not be published' }); }
      }
      return send(200, this.tagJson(name));
    }
    ...
    if (pathname === '/api/v1/accounts/update_credentials' || pathname === '/api/v1/profile') {
      ...
      if ('attribution_domains' in form || 'attribution_domains[]' in form) {
        try {
          cfg.attributionDomains = normalizeAttributionDomains(
            form.attribution_domains ?? form['attribution_domains[]'],
          );
      ...
      Object.assign(this.agent.publisher.config, {
        ...
        attributionDomains: cfg.attributionDomains || [],
      });
      ...
      const account = this.credentialAccount();
      return send(200, pathname === '/api/v1/profile'
        ? { ...account, attribution_domains: account.source.attribution_domains }
        : account);
    }
    ...
    if (pathname === '/api/v1/statuses' && req.method === 'POST') {
      ...
      const quotedUrl = body.quoted_status_id ? this.store.urlFor(String(body.quoted_status_id)) : null;
      const quotedStatus = quotedUrl
        ? this.store.getStatuses().find(x => x.noteId === quotedUrl) : null;
      if (body.quoted_status_id && !quotedStatus) return send(404, { error: 'Quoted status not found' });
      if (quotedStatus && inReplyTo) return send(422, { error: 'a status cannot be both a reply and a quote' });
      if (quotedStatus && visibility === 'direct') return send(422, { error: 'direct statuses cannot quote another status' });
      if (quotedStatus && (['private', 'direct'].includes(quotedStatus.visibility || '')
        || quotedStatus.nonPublic || quotedStatus.direct)) {
        return send(422, { error: 'followers-only and direct statuses cannot be quoted' });
      }
      if (quotedStatus && publication.objectType !== 'Note') return send(422, { error: 'a quote post must be a Note' });
      const quotePolicy = String(body.quote_approval_policy || 'public');
      if (!['public', 'followers', 'nobody'].includes(quotePolicy)) {
        return send(422, { error: 'quote_approval_policy must be public, followers, or nobody' });
      }
      ...
      const statusText = quotedStatus && !String(body.status).includes(quotedStatus.noteId)
        ? `${String(body.status).trim()}\n\n${quotedStatus.noteId}` : body.status;
      const note = await this.agent.publisher.publishNote(statusText,
        { inReplyTo, attachments, visibility, spoilerText, quotedStatus, quotePolicy, ...publication });
    ...
    if (mPin && req.method === 'POST') {
      ...
      if (!s) return send(404, { error: 'Record not found' });
      if (s.actor !== this.urls.actor) return send(403, { error: 'not your status' });
      const shouldPin = mPin[2] === 'pin';
      if (shouldPin && !s.pinned && this.store.getStatuses()
        .filter(x => x.actor === this.urls.actor && x.pinned).length >= MAX_PINNED_STATUSES) {
        return send(422, { error: `you may pin at most ${MAX_PINNED_STATUSES} statuses` });
      }
      const updated = this.store.updateStatus(s.noteId, { pinned: shouldPin });
      try { await this.agent.publisher.publishFeatured(); }
      catch (error) {
        this.store.updateStatus(s.noteId, { pinned: !!s.pinned });
        await this.agent.publisher.publishFeatured().catch(() => {});
        return send(502, { error: `the featured collection could not be published: ${error.message}` });
      }
      return send(200, this.status(updated || s));
    }

    const mQuotes = /^\/api\/v1\/statuses\/([a-f0-9]+)\/quotes$/.exec(pathname);
    if (mQuotes && req.method === 'GET') {
      const target = this.store.urlFor(mQuotes[1]);
      if (!target) return send(404, { error: 'Record not found' });
      const all = this.store.getStatuses();
      const { items, headers } = this.page(all.filter(s => s.quoteOf === target && s.quoteState === 'accepted'), url);
      return send(200, items.map(s => this.status(s, { all })), headers);
    }
    const mRevokeQuote = /^\/api\/v1\/statuses\/([a-f0-9]+)\/quotes\/([a-f0-9]+)\/revoke$/.exec(pathname);
    if (mRevokeQuote && req.method === 'POST') {
      const target = this.store.urlFor(mRevokeQuote[1]);
      const quoting = this.store.urlFor(mRevokeQuote[2]);
      const targetStatus = target && this.store.getStatuses().find(s => s.noteId === target);
      const quotingStatus = quoting && this.store.getStatuses().find(s => s.noteId === quoting);
      if (!targetStatus || !quotingStatus || quotingStatus.quoteOf !== target) return send(404, { error: 'Record not found' });
      if (targetStatus.actor !== this.urls.actor) return send(403, { error: 'not your status' });
      const authorizations = this.store.read('quote-authorizations.json', []);
      const authorization = authorizations.find(a => a.target === target && a.quote === quoting);
      if (authorization) {
        const removed = await this.agent.remote.delete(authorization.id).catch(() => false);
        if (!removed) return send(502, { error: 'quote authorization could not be revoked on the pod' });
        this.store.write('quote-authorizations.json', authorizations.filter(a => a.id !== authorization.id));
        const actor = await this.agent.intake.fetchAP(authorization.actor).catch(() => null);
        const inbox = actor?.endpoints?.sharedInbox || actor?.inbox;
        if (inbox) await this.agent.deliverer.deliver(inbox, {
          '@context': 'https://www.w3.org/ns/activitystreams',
          id: `${authorization.id}#delete`, type: 'Delete', actor: this.urls.actor,
          object: authorization.id, to: [authorization.actor],
        });
      }
      this.store.updateStatus(quoting, { quoteState: 'revoked', quoteAuthorization: null });
      return send(200, this.status(this.store.getStatuses().find(s => s.noteId === target)));
    }
    const mInteractionPolicy = /^\/api\/v1\/statuses\/([a-f0-9]+)\/interaction_policy$/.exec(pathname);
    if (mInteractionPolicy && req.method === 'PUT') {
      const noteUrl = this.store.urlFor(mInteractionPolicy[1]);
      const s = noteUrl && this.store.getStatuses().find(x => x.noteId === noteUrl);
      if (!s) return send(404, { error: 'Record not found' });
      if (s.actor !== this.urls.actor) return send(403, { error: 'not your status' });
      const body = await readBody(req);
      const policy = String(body.quote_approval_policy || '');
      if (!['public', 'followers', 'nobody'].includes(policy)) return send(422, { error: 'invalid quote_approval_policy' });
      const patched = this.store.updateStatus(s.noteId, { quotePolicy: policy });
      await this.agent.publisher.updateNote(patched, {
        content: patched.text ?? htmlToText(patched.content || ''),
        spoilerText: patched.spoiler || null, attachments: patched.attachments || [],
      });
      return send(200, this.status(this.store.getStatuses().find(x => x.noteId === s.noteId)));
    }
    ...
    // Suggestions are derived only from actors already present in this local
    // mirror. Dismissals are durable and neither blocked nor muted accounts can
    // leak back into recommendations.
    if ((pathname === '/api/v2/suggestions' || pathname === '/api/v1/suggestions') && req.method === 'GET') {
      const limit = Math.min(Math.max(Number(url.searchParams.get('limit')) || 40, 1), 80);
      const contacts = this.store.getContacts();
      const excluded = new Set([
        this.urls.actor, ...contacts.following.map(x => x.actor),
        ...this.store.getBlocklist().actors, ...this.store.getMuted().actors,
        ...this.store.getDismissedSuggestions(),
      ]);
      const activity = new Map();
      const knownActors = this.store.getActors();
      for (const s of this.store.getStatuses()) {
        if (s.actor && !activity.has(s.actor)) activity.set(s.actor, s.published || '');
      }
      const actors = Object.keys(knownActors).filter(a => !excluded.has(a))
        .sort((a, b) => String(activity.get(b) || knownActors[b]?.fetchedAt || '')
          .localeCompare(String(activity.get(a) || knownActors[a]?.fetchedAt || '')))
        .slice(0, limit);
      const accounts = actors.map(a => this.account(a));
      return send(200, pathname.startsWith('/api/v2/')
        ? accounts.map(account => ({ source: 'global', account })) : accounts);
    }
    const mDismissSuggestion = /^\/api\/v1\/suggestions\/([a-f0-9]+)$/.exec(pathname);
    if (mDismissSuggestion && req.method === 'DELETE') {
      const actor = this.store.urlFor(mDismissSuggestion[1]);
      if (actor) this.store.setDismissedSuggestions([...this.store.getDismissedSuggestions(), actor]);
      return send(200, {});
    }

    if (pathname === '/api/v1/collection_sources' && req.method === 'GET') {
      return send(200, COLLECTION_SOURCES);
    }
    if (pathname === '/api/v1/collection_sources/preview' && req.method === 'POST') {
      const body = await readBody(req);
      try {
        const pack = await this.collectionSourceLoader(body.url);
        return send(200, {
          name: pack.name, description: pack.description, source_url: pack.url,
          source_page: pack.page, source_kind: pack.kind, account_count: pack.accounts.length,
        });
      } catch (error) { return send(422, { error: error.message }); }
    }
    if (pathname === '/api/v1/collection_sources/import' && req.method === 'POST') {
      const body = await readBody(req);
      try { return send(200, await this.importCollection(body.url)); }
      catch (error) { return send(422, { error: error.message }); }
    }

    // Mastodon 4.6 Collections are recommendations, distinct from timeline
    // Lists. Collection ownership is always the authenticated local account.
    if (pathname === '/api/v1/collections' && req.method === 'POST') {
      const body = await readBody(req);
      const input = collectionInput(body);
      if (input.error) return send(422, { error: input.error });
      const collections = this.store.getCollections();
      if (collections.length >= MAX_COLLECTIONS) return send(422, { error: `at most ${MAX_COLLECTIONS} collections are allowed` });
      const id = crypto.randomBytes(8).toString('hex');
      const now = new Date().toISOString();
      const accountIds = [...new Set([].concat(body.account_ids || body['account_ids[]'] || []).map(String))].slice(0, 80);
      if (accountIds.length && !input.discoverable) {
        return send(422, { error: 'a private Collection must be made discoverable before inviting accounts' });
      }
      if (accountIds.some(accountId => !this.store.urlFor(accountId) || this.store.urlFor(accountId) === this.urls.actor)) {
        return send(422, { error: 'account_ids must identify known remote accounts' });
      }
      const items = accountIds.map(accountId => {
        const actor = this.store.urlFor(accountId);
        return actor && actor !== this.urls.actor ? {
          id: crypto.randomBytes(8).toString('hex'), accountId, actor,
          state: 'pending', createdAt: now,
        } : null;
      }).filter(Boolean);
      const c = {
        id, accountId: this.store.idFor(this.urls.actor),
        actor: this.urls.actor,
        uri: `${this.urls.collections}${id}`, url: `${this.urls.profileHtml}#collection-${id}`,
        ...input, items, createdAt: now, updatedAt: now,
      };
      collections.unshift(c);
      this.store.setCollections(collections);
      try { await this.agent.publisher.publishCollection(c); }
      catch (error) {
        this.store.setCollections(collections.filter(x => x.id !== id));
        return send(502, { error: `collection could not be published: ${error.message}` });
      }
      for (const item of c.items) {
        await this.agent.publisher.requestCollectionFeature(c, item)
          .catch(error => this.log(`collection feature request: ${error.message}`));
      }
      this.store.setCollections(collections);
      return send(200, { collection: collectionJson(c) });
    }
    const mCollection = /^\/api\/v1\/collections\/([a-f0-9]+)$/.exec(pathname);
    if (mCollection) {
      const collections = this.store.getCollections();
      const remoteCollections = this.store.getRemoteCollections();
      const c = collections.find(x => x.id === mCollection[1])
        || remoteCollections.find(x => x.id === mCollection[1]);
      if (!c) return send(404, { error: 'Record not found' });
      if (req.method === 'GET') {
        const actors = [...new Set([
          c.actor || this.urls.actor,
          ...(c.local === false ? [this.urls.actor] : []),
          ...(c.items || []).filter(i => i.state === 'accepted' || i.state === 'pending').map(i => i.actor),
        ])];
        return send(200, { accounts: actors.map(a => this.account(a)), collection: collectionJson(c) });
      }
      if (c.local === false) return send(403, { error: 'a remote Collection cannot be modified locally' });
      if (req.method === 'PATCH') {
        const body = await readBody(req);
        const input = collectionInput(body, c);
        if (input.error) return send(422, { error: input.error });
        ...
        try { await this.agent.publisher.publishCollection(c); }
          ...
          collections[collections.indexOf(c)] = previous;
          this.store.setCollections(collections);
          return send(502, { error: `collection could not be published: ${error.message}` });
        }
        return send(200, { collection: collectionJson(c) });
      ...
      if (req.method === 'DELETE') {
        const removed = await this.agent.publisher.remote.delete(c.uri).catch(() => false);
        if (!removed) return send(502, { error: 'collection could not be removed from the pod' });
        this.store.setCollections(collections.filter(x => x.id !== c.id));
        return send(200, {});
      }
    }
    const mCollectionItems = /^\/api\/v1\/collections\/([a-f0-9]+)\/items(?:\/([a-f0-9]+)(?:\/(revoke))?)?$/.exec(pathname);
    if (mCollectionItems) {
      const collections = this.store.getCollections();
      const remoteCollections = this.store.getRemoteCollections();
      const c = collections.find(x => x.id === mCollectionItems[1])
        || remoteCollections.find(x => x.id === mCollectionItems[1]);
      if (!c) return send(404, { error: 'Record not found' });
      c.items ||= [];
      if (c.local === false) {
        const item = c.items.find(i => i.id === mCollectionItems[2]);
        if (!item) return send(404, { error: 'Record not found' });
        if (req.method !== 'POST' || mCollectionItems[3] !== 'revoke') {
          return send(403, { error: 'only revocation is allowed for a remote Collection' });
        }
        if (item.actor !== this.urls.actor || !item.approvalUri) return send(403, { error: 'not your Collection item' });
        const removed = await this.agent.remote.delete(item.approvalUri).catch(() => false);
        if (!removed) return send(502, { error: 'feature authorization could not be revoked on the pod' });
        this.store.setRemoteCollections(remoteCollections.filter(x => x.id !== c.id));
        const owner = await this.agent.intake.fetchAP(c.actor).catch(() => null);
        const inbox = owner?.endpoints?.sharedInbox || owner?.inbox;
        if (inbox) await this.agent.deliverer.deliver(inbox, {
          '@context': 'https://www.w3.org/ns/activitystreams',
          id: `${item.approvalUri}#delete`, type: 'Delete', actor: this.urls.actor,
          object: item.approvalUri, to: [c.actor],
        });
        return send(200, {});
      }
      if (req.method === 'POST' && !mCollectionItems[2]) {
        if (!c.discoverable) {
          return send(422, { error: 'a private Collection must be made discoverable before inviting accounts' });
        }
        const body = await readBody(req);
        ...
        const actor = this.store.urlFor(accountId);
        if (!actor || actor === this.urls.actor) return send(422, { error: 'a valid remote account_id is required' });
        let item = c.items.find(i => i.actor === actor);
        if (!item) {
          const previous = structuredClone(c);
          item = { id: crypto.randomBytes(8).toString('hex'), accountId, actor, state: 'pending', createdAt: new Date().toISOString() };
          c.items.push(item);
        ...
        return send(200, { collection_item: collectionJson(c).items.find(i => i.id === item.id) });
      }
      const item = c.items.find(i => i.id === mCollectionItems[2]);
      if (!item) return send(404, { error: 'Record not found' });
      const previous = structuredClone(c);
      if (req.method === 'POST' && mCollectionItems[3] === 'revoke') {
        return send(403, { error: 'you cannot revoke another account from your own Collection; remove the item instead' });
      } else if (req.method === 'DELETE') c.items = c.items.filter(i => i.id !== item.id);
    ...
    const mAccountCollections = /^\/api\/v1\/accounts\/([a-f0-9]+)\/(collections|in_collections)$/.exec(pathname);
    if (mAccountCollections && req.method === 'GET') {
      const actor = this.store.urlFor(mAccountCollections[1]);
      if (!actor) return send(404, { error: 'Record not found' });
      let collections = this.store.getCollections();
      if (mAccountCollections[2] === 'collections') {
        collections = actor === this.urls.actor ? collections : collections.filter(c => c.discoverable && c.accountId === mAccountCollections[1]);
      } else {
        collections = actor === this.urls.actor
          ? this.store.getRemoteCollections()
          : collections.filter(c => (c.items || []).some(i => i.actor === actor && i.state === 'accepted'));
      }
      const offset = Math.max(Number(url.searchParams.get('offset')) || 0, 0);
      const limit = Math.min(Math.max(Number(url.searchParams.get('limit')) || 40, 1), 80);
      return send(200, { collections: collections.slice(offset, offset + limit).map(collectionJson) });
    }
    ...
    const mAccountFeatured = /^\/api\/v1\/accounts\/([a-f0-9]+)\/featured_tags$/.exec(pathname);
    if (mAccountFeatured && req.method === 'GET') {
      const actorUrl = this.store.urlFor(mAccountFeatured[1]);
      if (!actorUrl) return send(404, { error: 'Record not found' });
      return send(200, actorUrl === this.urls.actor
        ? this.featuredTags().map(t => this.featuredTagJson(t)) : []);
    }
    ...
    if (pathname === '/api/v2/search' || pathname === '/api/v1/search') {
      ...
      const out = { accounts: [], statuses: [], hashtags: [], collections: [] };
      ...
      if (asHandle && type !== 'statuses') {
        ...
        if (type !== 'accounts' && type !== 'statuses') {
          const names = new Set();
          for (const status of this.store.getStatuses()) {
            for (const name of this.statusTagNames(status)) {
              if (name.includes(needle.replace(/^#/, ''))) names.add(name);
            }
          }
          out.hashtags = [...names].slice(0, 20).map(name => this.tagJson(name));
          out.collections = this.store.getCollections()
            .filter(c => c.discoverable && `${c.name} ${c.description}`.toLowerCase().includes(needle))
            .slice(0, 20).map(collectionJson);
        }
      }
      return send(200, out);
    }
    ...
    req.on('end', () => {
      ...
      try {
        ...
        for (const [key, value] of new URLSearchParams(data)) {
          ...
          else form[key] = [].concat(form[key], value);
        }
        resolve(form);
      } catch (e) { reject(e); }
    });
