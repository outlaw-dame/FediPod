...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    if (pathname === '/api/v1/ailo/ai/moderation/suggest' && req.method === 'POST') {
      ...
      const { domains, actors: blockedActors } = this.store.getBlocklist();
      const mutedActors = this.store.getMuted().actors;
      const domainCandidates = candidateModerationDomains({ blockedActors, mutedActors, blockedDomains: domains });
      try {
        const suggestions = await picked.backend.suggestModeration({
          apiKey: picked.apiKey, filterTitles, blockedDomains: domains, mutedCount: mutedActors.length, domainCandidates,
        });
    ...
    if (pathname === '/api/v1/ailo/ai/moderation/summarize' && req.method === 'POST') {
      const body = await readBody(req);
      const picked = aiBackendFor(this.store, body.provider);
      if (picked.error) return send(503, { error: picked.error });
      try {
        const summary = await picked.backend.summarizeModeration({ apiKey: picked.apiKey, stats: body.stats || {} });
        return send(200, { summary });
      } catch (e) {
        this.log(`ai moderation summarize failed: ${e.message}`);
        return send(502, { error: e.message });
      }
    }
    ...
    if (pathname === '/api/v1/ailo/moderation/intake-stats' && req.method === 'GET') {
      // How much content a block/domain-block actually kept out, not just
      // that one exists — see store.mjs's recordModerationEvent for why this
      // can only be counted here: a blocked actor's post never reaches Ailo,
      // so the client has nothing of its own to count for this part of the
      // weekly summary (it does track its own keyword-filter hits).
      const days = Math.min(30, Math.max(1, Number(url.searchParams.get('days')) || 7));
      return send(200, this.store.getModerationStats(days));
    }
