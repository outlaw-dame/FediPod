...
import {
  canAddCustomFeed, customFeedJson, normalizeAccount, normalizeCustomFeed, normalizeDomain,
  statusIsVisible,
...
function aiBackendFor(store, requestedProvider) {
...
export class MastoApi {
  ...
  async handle(req, res, pathname, url) {
    ...
    if (mCustomTimeline && req.method === 'GET') {
      ...
      const excludedAccounts = new Set(feed.excludeAccounts || []);
      const excludedWords = feed.excludeWords || [];
      const all = this.store.getStatuses();
      const items = all.filter((status) => statusIsVisible(this.store, status)).filter((status) => {
        const account = normalizeAccount(this.account(status.actor).acct);
        if (excludedAccounts.has(account)) return false;
        const text = htmlToText(status.content || status.text || '').toLocaleLowerCase();
