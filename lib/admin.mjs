...
import { CreatorPreviewService } from './creator-attribution.mjs';
import { ProviderSecretStore } from './provider-secrets.mjs';
import { AiService, SafeBrowsingService } from './ai.mjs';
...
export function startAdmin({ port, gateToken, agent, log = console.log, handle = null }) {
  ...
  const creatorPreview = new CreatorPreviewService({ agent, log });
  const providerSecrets = new ProviderSecretStore({ home: agent.home });
  const ai = new AiService({ credentials: providerSecrets });
  const safeBrowsing = new SafeBrowsingService({ credentials: providerSecrets });
  const masto = new MastoApi({ agent, log, allowed, creatorPreview, ai, safeBrowsing, providerSecrets });
