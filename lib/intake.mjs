...
// For recordModerationEvent's domain breakdown — best-effort: isBlocked()
// already treats an unparsable URL as blocked, so this can still be called
// on one; null just means that refusal contributes to the totals but not
// to "top domains".
function hostOf(u) {
...
export class Intake {
  ...
  async handle(activity) {
    ...
    if (this.store.isBlocked(actor)) {
      this.store.recordModerationEvent('blocked-actor', hostOf(actor));
  ...
  async onCreate(activity, actor) {
    ...
    if (this.store.isBlocked(objectId)) {
      this.store.recordModerationEvent('blocked-domain', hostOf(objectId));
  ...
  async ingestNote(objectId, actor, { via } = {}) {
    ...
    if (this.store.isBlocked(author)) {
      this.store.recordModerationEvent('blocked-actor', hostOf(author));
