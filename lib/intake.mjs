...
import { AS_CTX, PUBLIC } from './wire.mjs';
...
const SOURCE_TYPES = new Set(['text/markdown', 'text/x.misskeymarkdown']);
const MAX_SOURCE_CHARS = 100_000;

export function sourceOf(note) {
  const mediaType = String(note?.source?.mediaType || '').toLowerCase();
  const content = note?.source?.content;
  if (SOURCE_TYPES.has(mediaType) && typeof content === 'string' && content.length <= MAX_SOURCE_CHARS) {
    return { content, mediaType };
  }
  // Older Misskey objects may only carry the historical extension field.
  if (typeof note?._misskey_content === 'string' && note._misskey_content.length <= MAX_SOURCE_CHARS) {
    return { content: note._misskey_content, mediaType: 'text/x.misskeymarkdown' };
  }
  return null;
}

function titleOf(note) {
  if (note?.type !== 'Article' || typeof note.name !== 'string') return null;
  return note.name.replace(/<[^>]*>/g, '').trim().slice(0, 300) || null;
}
...
export class Intake {
  ...
  async handle(activity) {
    ...
    switch (activity.type) {
      ...
      case 'QuoteRequest': return this.onQuoteRequest(activity, actor);
      case 'FeatureRequest': return this.onFeatureRequest(activity, actor);
  ...
  async ingestNote(objectId, actor, { via } = {}) {
    ...
    const quoteOf = typeof note.quote === 'string' ? note.quote : note.quote?.id;
    const quoteAuthorization = typeof note.quoteAuthorization === 'string'
      ? note.quoteAuthorization : note.quoteAuthorization?.id;
    this.store.addStatus({
      noteId: note.id, actor: author, content,
      published: note.published, inReplyTo: note.inReplyTo, kind,
      objectType: note.type, contentType: source?.mediaType || 'text/plain',
      ...(title ? { title } : {}),
      ...(source ? { source } : {}),
      ...(direct ? { direct: true } : {}),
      ...
      ...(via ? { via } : {}),
      ...(quoteOf ? {
        quoteOf, quoteAuthorization: quoteAuthorization || null,
        quoteState: quoteAuthorization || author === this.urls.actor ? 'accepted' : 'pending',
      } : {}),
    });
    if (quoteOf?.startsWith(this.urls.notes) && quoteAuthorization) {
      this.store.addNotification({ type: 'quote', actor: author, noteId: note.id });
    }
  ...
  async onUpdate(activity, actor) {
    ...
    this.store.updateStatus(objectId, {
      ...
      objectType: note.type, contentType: source?.mediaType || 'text/plain',
      title: title || undefined, source: source || undefined,
  ...
  async onReject(activity, actor) {
    ...
    const collection = collections.find(c => (c.items || []).some(i => i.requestId === named));
    const feature = collection?.items.find(i => i.requestId === named);
    if (feature) {
      if (feature.actor !== actor) return 'feature rejection came from the wrong actor';
      feature.state = 'rejected';
      collection.updatedAt = new Date().toISOString();
      this.store.setCollections(collections);
      await this.publisher.publishCollection(collection);
      this.log(`collection feature rejected by ${actor}`);
      return;
    }
    const quote = named && this.store.getStatuses().find(s => s.quoteRequestId === named);
    if (quote) {
      const target = this.store.getStatuses().find(s => s.noteId === quote.quoteOf);
      if (!target || target.actor !== actor) return 'quote rejection came from the wrong actor';
      const patched = this.store.updateStatus(quote.noteId, { quoteState: 'rejected', quoteAuthorization: null });
      await this.publisher.updateNote(patched, {
        content: patched.text ?? String(patched.content || '').replace(/<[^>]*>/g, ''),
        spoilerText: patched.spoiler || null, attachments: patched.attachments || [],
      });
      this.log(`quote rejected by ${actor}`);
      return;
    }
  ...
  async onAccept(activity, actor) {
    ...
    const feature = collection?.items.find(i => i.requestId === named);
    if (feature) {
      const result = typeof activity.result === 'string' ? activity.result : activity.result?.id;
      if (feature.actor !== actor) return 'feature acceptance came from the wrong actor';
      if (!result || !sameOrigin(result, actor)) return 'feature acceptance has no authorization at the featured actor origin';
      const authorization = await this.fetchAP(result);
      const attributedTo = typeof authorization?.attributedTo === 'string'
        ? authorization.attributedTo : authorization?.attributedTo?.id;
      const interactingObject = typeof authorization?.interactingObject === 'string'
        ? authorization.interactingObject : authorization?.interactingObject?.id;
      const interactionTarget = typeof authorization?.interactionTarget === 'string'
        ? authorization.interactionTarget : authorization?.interactionTarget?.id;
      if (!authorization || !['FeatureAuthorization', 'https://w3id.org/fep/7aa9#FeatureAuthorization'].includes(authorization.type)
        || attributedTo !== actor || interactingObject !== collection.uri || interactionTarget !== actor) {
        return 'feature authorization does not match the request';
      }
      feature.state = 'accepted';
      feature.approvalUri = result;
      collection.updatedAt = new Date().toISOString();
      this.store.setCollections(collections);
      await this.publisher.publishCollection(collection);
      this.log(`collection feature accepted by ${actor}`);
      return;
    }
    const quote = named && this.store.getStatuses().find(s => s.quoteRequestId === named);
    if (quote) {
      const target = this.store.getStatuses().find(s => s.noteId === quote.quoteOf);
      const result = typeof activity.result === 'string' ? activity.result : activity.result?.id;
      if (!target || target.actor !== actor) return 'quote acceptance came from the wrong actor';
      if (!result || !sameOrigin(result, actor)) return 'quote acceptance has no authorization at the quoted actor origin';
      const authorization = await this.fetchAP(result);
      const attributedTo = typeof authorization?.attributedTo === 'string'
        ? authorization.attributedTo : authorization?.attributedTo?.id;
      const interactingObject = typeof authorization?.interactingObject === 'string'
      ...
      const interactionTarget = typeof authorization?.interactionTarget === 'string'
        ? authorization.interactionTarget : authorization?.interactionTarget?.id;
      if (!authorization || !['QuoteAuthorization', 'https://w3id.org/fep/044f#QuoteAuthorization'].includes(authorization.type)
        || attributedTo !== actor || interactingObject !== quote.noteId || interactionTarget !== quote.quoteOf) {
        return 'quote authorization does not match the request';
      }
      const patched = this.store.updateStatus(quote.noteId, {
        quoteState: 'accepted', quoteAuthorization: result,
      });
      await this.publisher.updateNote(patched, {
        content: patched.text ?? String(patched.content || '').replace(/<[^>]*>/g, ''),
        spoilerText: patched.spoiler || null, attachments: patched.attachments || [],
      });
      this.log(`quote accepted by ${actor}`);
      return;
    }
    const contacts = this.store.getContacts();
    const rec = contacts.following.find(f => f.actor === actor);
  ...
  async onQuoteRequest(activity, actor) {
    const target = typeof activity.object === 'string' ? activity.object : activity.object?.id;
    const instrument = typeof activity.instrument === 'string' ? activity.instrument : activity.instrument?.id;
    const own = target && this.store.getStatuses().find(s => s.noteId === target && s.actor === this.urls.actor);
    if (!own || !instrument || !sameOrigin(instrument, actor)) return 'quote request does not name a local status and actor-owned quote';
    const actorDoc = await this.fetchAP(actor);
    if (!actorDoc?.inbox || actorDoc.id !== actor) return 'quote requester could not be verified';
    const quoting = await this.fetchAP(instrument);
    const quotingActor = quoting && authorOf(quoting, actor);
    const quoted = typeof quoting?.quote === 'string' ? quoting.quote : quoting?.quote?.id;
    if (!quoting || quoting.id !== instrument || quotingActor !== actor || quoted !== target) {
      return 'quoting status does not match the request';
    }
    const policy = own.quotePolicy || 'public';
    const follower = this.store.getContacts().followers.some(f => f.actor === actor);
    const allowed = !own.nonPublic && !own.direct && !['private', 'direct'].includes(own.visibility || '')
      && (policy === 'public' || (policy === 'followers' && follower));
    if (!allowed) {
      await this.deliverer.deliver(actorDoc.inbox, {
        '@context': AS_CTX, id: `${this.urls.actor}#reject-quote-${this.serial++}`,
        type: 'Reject', actor: this.urls.actor, object: activity, to: [actor],
      });
      return;
    }
    const records = this.store.read('quote-authorizations.json', []);
    const existing = records.find(r => r.actor === actor && r.quote === instrument && r.target === target);
    if (!existing && records.length >= 1000) return 'quote authorization limit reached';
    let id = existing?.id;
    if (!id) {
      const crypto = await import('node:crypto');
      id = this.urls.quoteAuthorizations + crypto.randomBytes(12).toString('hex');
      const { quoteAuthorizationDoc } = await import('./wire.mjs');
      const authorization = quoteAuthorizationDoc({ id, actor: this.urls.actor, quote: instrument, target });
      await this.remote.putJson(id, authorization);
      await this.remote.setAcl(id, ['Read']);
      records.unshift({ id, actor, quote: instrument, target, at: new Date().toISOString() });
      this.store.write('quote-authorizations.json', records);
    }
    await this.deliverer.deliver(actorDoc.inbox, {
      '@context': AS_CTX, id: `${this.urls.actor}#accept-quote-${this.serial++}`,
      type: 'Accept', actor: this.urls.actor, object: activity, result: id, to: [actor],
    });
    this.store.addNotification({ type: 'quote', actor, noteId: instrument });
    this.log(`quote authorized: ${instrument}`);
  }

  async onFeatureRequest(activity, actor) {
    const target = typeof activity.object === 'string' ? activity.object : activity.object?.id;
    const instrument = typeof activity.instrument === 'string'
      ? activity.instrument : activity.instrument?.id;
    if (target !== this.urls.actor || !instrument || !sameOrigin(instrument, actor)) {
      return 'feature request does not name this actor and an actor-owned collection';
    }

    const actorDoc = await this.fetchAP(actor);
    if (!actorDoc?.inbox || actorDoc.id !== actor) return 'collection owner could not be verified';
    const collection = await this.fetchAP(instrument);
    const collectionType = [].concat(collection?.type || []);
    const attributedTo = typeof collection?.attributedTo === 'string'
      ? collection.attributedTo : collection?.attributedTo?.id;
    if (!collection || collection.id !== instrument
      || !collectionType.some(t => t === 'FeaturedCollection' || t === 'https://w3id.org/fep/7aa9#FeaturedCollection')
      || attributedTo !== actor) {
      return 'featured collection does not match the request';
    }

    // This actor is public by design. A blocked owner was rejected before the
    // handler, and a collection explicitly hidden from discovery is not a
    // recommendation we should publish back through the client API.
    if (collection.discoverable === false) {
      await this.deliverer.deliver(actorDoc.inbox, {
        '@context': AS_CTX, id: `${this.urls.actor}#reject-feature-${this.serial++}`,
        type: 'Reject', actor: this.urls.actor, object: activity, to: [actor],
      });
      return;
    }
    ...
    let saved = remoteCollections.find(c => c.uri === instrument);
    if (!saved && remoteCollections.length >= 200) return 'remote collection limit reached';
    let authorizationId = saved?.items?.find(i => i.actor === this.urls.actor)?.approvalUri;
    if (!authorizationId) {
      const crypto = await import('node:crypto');
      authorizationId = this.urls.featureAuthorizations + crypto.randomBytes(12).toString('hex');
      const { featureAuthorizationDoc } = await import('./wire.mjs');
      await this.remote.putJson(authorizationId, featureAuthorizationDoc({
        id: authorizationId, actor: this.urls.actor, collection: instrument,
      }));
      await this.remote.setAcl(authorizationId, ['Read']);
    }

    const now = new Date().toISOString();
    const item = {
      id: this.store.idFor(`${instrument}#${this.urls.actor}`),
      accountId: this.store.idFor(this.urls.actor), actor: this.urls.actor,
      state: 'accepted', approvalUri: authorizationId, createdAt: now,
    };
    const next = {
      id: saved?.id || this.store.idFor(instrument), accountId: this.store.idFor(actor),
      actor, uri: instrument, url: collection.url || instrument,
      name: String(collection.name || '').slice(0, 200),
      description: String(collection.summary || '').slice(0, 2000),
      language: collection.language || null, local: false,
      sensitive: !!collection.sensitive, discoverable: collection.discoverable !== false,
      items: [item], createdAt: saved?.createdAt || collection.published || now,
      updatedAt: collection.updated || now,
    };
    if (saved) remoteCollections[remoteCollections.indexOf(saved)] = next;
    else remoteCollections.unshift(next);
    this.store.setRemoteCollections(remoteCollections);

    await this.deliverer.deliver(actorDoc.inbox, {
      '@context': AS_CTX, id: `${this.urls.actor}#accept-feature-${this.serial++}`,
      type: 'Accept', actor: this.urls.actor, object: activity,
      result: authorizationId, to: [actor],
    });
    this.store.addNotification({ type: 'added_to_collection', actor, collectionId: next.id });
    this.log(`collection feature authorized: ${instrument}`);
  }
}
