...
test('recordModerationEvent + getModerationStats count events within the window', () => {
  ...
  assert.deepEqual(s.getModerationStats(7), { blockedActor: 2, blockedDomain: 1, topDomains: [] });
});
...
test('getModerationStats ignores events older than the requested window', () => {
  ...
  assert.deepEqual(s.getModerationStats(7), { blockedActor: 1, blockedDomain: 1, topDomains: [] });
  assert.deepEqual(s.getModerationStats(30), { blockedActor: 2, blockedDomain: 1, topDomains: [] });
});

test('getModerationStats ranks top domains by refusal count, within the window only', () => {
  const s = store();
  const now = Date.now();
  const DAY = 24 * 60 * 60 * 1000;
  s.write('moderation-events.json', [
    { ts: now, kind: 'blocked-actor', domain: 'spam.example' },
    { ts: now, kind: 'blocked-actor', domain: 'spam.example' },
    { ts: now, kind: 'blocked-domain', domain: 'spam.example' },
    { ts: now, kind: 'blocked-domain', domain: 'junk.social' },
    { ts: now - 10 * DAY, kind: 'blocked-domain', domain: 'old.example' },   // outside the window
    { ts: now, kind: 'blocked-actor', domain: null },   // an unparsable URL — no domain to count
  ]);
  assert.deepEqual(s.getModerationStats(7).topDomains, [
    { domain: 'spam.example', count: 3 },
    { domain: 'junk.social', count: 1 },
  ]);
});
