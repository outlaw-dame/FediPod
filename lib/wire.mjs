...
export function apUrls(remotePod, root = 'activitypods-js/') {
  ...
  return {
    ...
    collections: home + 'ap/collections/',
    quoteAuthorizations: home + 'ap/quote-authorizations/',
    featureAuthorizations: home + 'ap/feature-authorizations/',
    featureRequests: home + 'ap/feature-requests/',
...
export function actorDoc({ urls, handle, name, publicKeyPem, movedTo = null, kind = 'person',
  ...
  return {
    ...
    // Domains that may use <meta name="fediverse:creator"> to credit this
    // actor. Mastodon models this as a set of domain-name strings, not IRIs.
    ...(attributionDomains.length ? { attributionDomains } : {}),
...
export function noteDoc({ urls, slug, content, published, inReplyTo, attachments, mentions = [],
  ...
  quote = null, quoteAuthorization = null, quotePolicy = 'public' }) {
  ...
  const quoteContext = {
    ...
    interactionPolicy: 'gts:interactionPolicy', canQuote: 'gts:canQuote',
    automaticApproval: { '@id': 'gts:automaticApproval', '@type': '@id' },
  };
  const automaticApproval = quotePolicy === 'public' ? [PUBLIC]
    : quotePolicy === 'followers' ? [urls.followers] : [];
  const note = {
    '@context': [AS_CTX, quoteContext],
    id, type: objectType,
  ...
  if (quote) note.quote = quote;
  if (quoteAuthorization) note.quoteAuthorization = quoteAuthorization;
...
export function quoteRequestActivity({ urls, quote, target, recipient }) {
  return {
    '@context': [AS_CTX, { QuoteRequest: 'https://w3id.org/fep/044f#QuoteRequest' }],
    id: quote.id + '-quote-request', type: 'QuoteRequest', actor: urls.actor,
    object: target, instrument: quote.id, to: [recipient],
  };
}

export function quoteAuthorizationDoc({ id, actor, quote, target }) {
  return {
    '@context': [AS_CTX, {
      QuoteAuthorization: 'https://w3id.org/fep/044f#QuoteAuthorization',
      interactingObject: { '@id': 'https://w3id.org/fep/044f#interactingObject', '@type': '@id' },
      interactionTarget: { '@id': 'https://w3id.org/fep/044f#interactionTarget', '@type': '@id' },
    }],
    id, type: 'QuoteAuthorization', attributedTo: actor,
    interactingObject: quote, interactionTarget: target,
  };
}
...
export function featureRequestActivity({ urls, id, collection, account }) {
  return {
    '@context': [AS_CTX, FEATURE_CONTEXT], id, type: 'FeatureRequest', actor: urls.actor,
    object: account, instrument: collection, to: [account],
  };
}

export function featureAuthorizationDoc({ id, actor, collection }) {
  return {
    '@context': [AS_CTX, FEATURE_CONTEXT], id, type: 'FeatureAuthorization',
    attributedTo: actor, interactingObject: collection, interactionTarget: actor,
  };
}

export function featuredCollectionDoc(c, acceptedItems) {
  return {
    '@context': [AS_CTX, FEATURE_CONTEXT], id: c.uri, type: 'FeaturedCollection',
    attributedTo: c.actor, url: c.url, name: c.name, summary: c.description || '',
    sensitive: !!c.sensitive, discoverable: !!c.discoverable,
    totalItems: acceptedItems.length, published: c.createdAt, updated: c.updatedAt,
    orderedItems: acceptedItems.map(i => ({
      id: `${c.uri}/items/${i.id}`, type: 'FeaturedItem', featuredObject: i.actor,
      featureAuthorization: i.approvalUri, published: i.createdAt,
    })),
  };
}
