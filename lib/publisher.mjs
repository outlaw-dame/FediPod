...
export class Publisher {
  ...
  async publishProfile({ force = false } = {}) {
    ...
    const actorDoc = wire.actorDoc({
      ...
      attributionDomains: this.config.attributionDomains || [],
  ...
  async requestCollectionFeature(c, item, { queueOnly = false, queue = null } = {}) {
    const handle = this.store.handleOf(item.actor)?.replace(/^@/, '');
    const actor = item.inbox ? null : handle && this.resolveMention
      ? await this.resolveMention(handle).catch(() => null) : null;
    const inbox = item.inbox || actor?.inbox;
    ...
    await this.remote.putJson(request.id, request);
    if (queueOnly) {
      const pending = queue || this.store.getQueue();
      pending.push({ inbox, activity: request, attempts: 0, nextAt: Date.now() });
      if (!queue) this.store.setQueue(pending);
    } else await this.deliverer.deliverToAll([inbox], request);
