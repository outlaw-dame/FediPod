...
export class Publisher {
  ...
  async publishCollections(which = ALL_COLLECTIONS) {
    ...
    if (which.following) {
      // hidden (social.mjs's followActor, e.g. relay-tags.mjs's per-hashtag
      // relay follows) is a real Follow but not a social one — it stays out
      // of the collection a stranger's client would read as "who they follow".
      await this.remote.putJson(urls.following,
        wire.orderedCollection(urls.following, contacts.following.filter(f => f.accepted && !f.hidden).map(f => f.actor)));
